...
(() => {
  ...
  function normalizeFeedItem(raw, wallStatus) {
    ...
    let buckets = raw.suggested_style_buckets || raw.buckets || [];
    if (typeof buckets === "string") {
      try { buckets = JSON.parse(buckets); } catch (_) { buckets = []; }
    }
    ...
    const item = {
      ...
      _raw_id: raw.id || "",
      source: raw.source || "",
      page_url: raw.page_url || raw.url || raw.link || "",
      query_used: raw.query_used || "",
      suggested_style_buckets: buckets,
    };
  ...
  async function loadMainWallText() {
    // Prefer authoritative plain jsonl first (avoid stale gz.b64 / split leftovers)
    const full = await fetch("data/l2_main_wall.jsonl");
    if (full.ok) {
    ...
    if (text) return text;
    const gz = await fetch("data/l2_main_wall.jsonl.gz.b64");
    if (gz.ok) {
      const b64 = (await gz.text()).trim();
  ...
  async function loadLiveFeeds() {
    ...
    const mainRes = { ok: true, status: 200, text: async () => mainText };
    if (bucketsRes.ok) {
    ...
    if (!state.wallItems.length && mainRaw.length) {
      // Last-resort: show normalized rows even if fields look odd
      state.wallItems = mainRaw.map((r) => normalizeFeedItem(r, "main_wall"));
    }
  ...
  function getFilteredWallItems() {
    ...
    function applyBriefOrder(list) {
      if (state.onlyBriefRelevant) {
        const matched = list.filter((it) => scoreBriefRelevance(it) === "match");
        // Never blank the wall when counts say we have a main wall
  ...
  async function boot() {
    ...
    try {
      ...
      // 2) Product pack for L1 brief + L4 strategy cards (no demo fallback)
      let res = await fetch("data/product-bundle.json");
